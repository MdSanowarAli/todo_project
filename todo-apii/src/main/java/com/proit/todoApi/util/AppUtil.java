...
/*	public static String getBeanToJsonString(Object beanClass) {
	...
	public static String oracleBuildPaginatedQuery(String baseQuery, PaginationCriteria paginationCriteria) {
		//String queryTemplate = "SELECT FILTERED_ORDERD_RESULTS.* FROM (SELECT BASEINFO.* FROM ( %s ) BASEINFO %s  %s ) FILTERED_ORDERD_RESULTS LIMIT %d, %d";
		StringBuilder sb = new StringBuilder("SELECT FILTERED_ORDERD_RESULTS.* FROM (SELECT BASEINFO.* FROM ( #BASE_QUERY# ) BASEINFO #WHERE_CLAUSE#  #ORDER_CLASUE# ) FILTERED_ORDERD_RESULTS WHERE FILTERED_ORDERD_RESULTS.srlno > #PAGE_NUMBER# AND FILTERED_ORDERD_RESULTS.srlno <= #PAGE_SIZE#");
		String finalQuery = null;
	    Integer pageSize  = paginationCriteria.getPageNumber().intValue() +  paginationCriteria.getPageSize().intValue();
	 
		if(!AppUtil.isObjectEmpty(paginationCriteria)) {
			finalQuery = sb.toString().replaceAll("#BASE_QUERY#", baseQuery)
							.replaceAll("#WHERE_CLAUSE#", ((AppUtil.isObjectEmpty(paginationCriteria.getOracleFilterByClause())) ? "" : " WHERE ") + paginationCriteria.getOracleFilterByClause())
								.replaceAll("#ORDER_CLASUE#", paginationCriteria.getOrderByClause())
									.replaceAll("#PAGE_NUMBER#", paginationCriteria.getPageNumber().toString())
